...
const GoogleButton: React.FC<GoogleButtonProps> = ({ label = 'Continue with Google', className = '', ...buttonProps }) => {
  return (
    <button
      type="button"
      {...buttonProps}
      className={`w-full flex items-center justify-center gap-3 border border-slate-200 dark:border-slate-700 rounded-xl py-3 font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 shadow-sm transition-colors ${className}`}
    >
